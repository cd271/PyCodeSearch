...
def get_code_embeddings(code):
    tokens_ids = clone_detection_model.tokenize([code], max_length=512, mode="<encoder-only>")
    source_ids = torch.tensor(tokens_ids).to(device)
    _, embeddings = clone_detection_model(source_ids)

    return embeddings

def get_cos_similarity(query_embedding, code_embedding):
    cosine_sim = CosineSimilarity(dim=1, eps=1e-8)
    return cosine_sim(query_embedding, torch.stack(code_embedding))
